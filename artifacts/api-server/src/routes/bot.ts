...
router.get("/api/subscription/status", async (req, res) => {
  ...
  try {
    ...
    const isActive =
      sub.status === "active" && (sub.plan === "lifetime" || !sub.expiresAt || new Date(sub.expiresAt) > new Date());
...
router.post("/api/subscription/claim", async (req, res) => {
  const clientId = req.headers["x-client-id"] as string;
  if (!clientId) {
    res.status(400).json({ error: "X-Client-ID header required" });
    return;
...
router.post("/api/telegram/webhook", async (req, res) => {
  ...
  if (msg?.text?.startsWith("/start")) {
    ...
    const firstName = (msg.from?.first_name as string) || "there";
    try {
  ...
  const data: string = (callback.data as string) ?? "";
...
router.get("/api/telegram/setup", async (req, res) => {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  const appUrl =
    process.env.APP_URL ??
    (process.env.VERCEL_PROJECT_PRODUCTION_URL ? `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}` : null) ??
...
router.get("/api/conversations", async (req, res) => {
  ...
  try {
    ...
    if (clientId) {
      ...
      rows = await db.select().from(conversationsTable).orderBy(asc(conversationsTable.createdAt));
    }
...
router.post("/api/conversations/:id/messages", async (req, res) => {
  ...
  try {
    ...
    for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
      ...
      const toolCalls = Object.entries(toolCallMap).map(([, tc]) => tc);
      ...
      toolMessages = [...toolMessages, assistantToolCallsMsg];
      ...
      res.write(`data: ${JSON.stringify({ tool_use: toolNames })}\n\n`);
    }
